...
DEBUG_STACK_WAVELET_TRACE = True
...
class TDMosaicApp:
    def __init__(self, cube_path: Path, cube_axis_order: str = "TYX"):
        ...
        self.autosave_path = (
            Path(__file__).resolve().parent / f"{self.cube_path.stem}_td_session_autosave.json"
        )
    ...
    def _export_current_map_fits(self) -> None:
        try:
            ...
            return
        self._refresh_saved_fits_browser()
        self._set_status(f"Saved current map FITS to {save_path}.")
    ...
    def _export_cut_ids_fits(self, cut_ids: list[int], label: str) -> None:
        ...
        self._refresh_saved_fits_browser()
        self._set_status(
            f"Saved {written} cut FITS file(s) for {label} in {export_dir}."
        )
    ...
    def _export_trace_ids_fits(self, cut_ids: list[int], label: str) -> None:
        ...
        self._refresh_saved_fits_browser()
        self._set_status(
            f"Saved {written} trace FITS file(s) for {label} in {export_dir}."
        )
    ...
    def _build_sidebar(self) -> None:
        ...
        self.ttk.Button(
            export_map_box,
            text="Save current map as FITS",
            command=self._export_current_map_fits,
        ).grid(row=0, column=0, sticky="ew")
        self.ttk.Label(
            export_map_box,
            text="Saves the cube frame at the current visual time with the cut coordinates at that t.",
            justify="left",
            wraplength=320,
        ...
        self.ttk.Label(
            export_cut_box,
            text="Each FITS keeps the TD map, time axis, distance axis, cut geometry, and event tables if available.",
            justify="left",
            wraplength=320,
        ...
        self.ttk.Label(
            export_trace_box,
            text="Writes one FITS per event/trace with point-by-point time index, distance and XY coordinates on the map.",
            justify="left",
            wraplength=320,
        ).grid(row=2, column=0, columnspan=2, sticky="w", pady=(6, 0))
